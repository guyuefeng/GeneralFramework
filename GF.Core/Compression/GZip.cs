...
namespace GF.Core.Compression
{
    public class GZip
    {
        ...
        public GZipResult Compress(string lpSourceFolder, string lpDestFolder, string zipFileName)
        {
        ...
        public GZipResult Compress(string lpSourceFolder, string searchPattern, SearchOption searchOption, string lpDestFolder, string zipFileName, bool deleteTempFile)
        {
        ...
        /// <param name="lpDestFolder">Folder to write the zip file into</param>
        /// <param name="zipFileName">Name of the zip file to write</param>
        public GZipResult Compress(IList<FileInfo> files, string[] folders, string lpBaseFolder, string lpDestFolder, string zipFileName)
        {
            ...
            return Compress(list, lpBaseFolder, lpDestFolder, zipFileName, true);
        }
        ...
        /// <param name="lpDestFolder">Folder to write the zip file into</param>
        /// <param name="zipFileName">Name of the zip file to write</param>
        public GZipResult Compress(IList<FileInfo> files, string lpBaseFolder, string lpDestFolder, string zipFileName)
        {
        ...
        /// <param name="zipFileName">Name of the zip file to write</param>
        /// <param name="deleteTempFile">Boolean, true deleted the intermediate temp file, false leaves the temp file in lpDestFolder (for debugging)</param>
        public GZipResult Compress(IList<FileInfo> files, string lpBaseFolder, string lpDestFolder, string zipFileName, bool deleteTempFile)
        {
        ...
        private void CreateZipFile(string lpSourceFile, string lpZipFile, GZipResult result)
        {
            byte[] buffer;
            int count = 0;
        ...
        private void CreateTempFile(IList<FileInfo> files, string lpBaseFolder, string lpTempFile, GZipResult result)
        {
            byte[] buffer;
            int count = 0;
            byte[] header;
            ...
            if (files != null && files.Count > 0)
            {
                try
                {
                    result.Files = new GZipFileInfo[files.Count];
        ...
        public GZipResult Decompress(string lpSourceFolder, string lpDestFolder, string zipFileName)
        {
            return Decompress(lpSourceFolder, lpDestFolder, zipFileName, true, true, null, null, 4096);
        }

        public GZipResult Decompress(string lpSourceFolder, string lpDestFolder, string zipFileName, bool writeFiles, string addExtension)
        {
            return Decompress(lpSourceFolder, lpDestFolder, zipFileName, true, writeFiles, addExtension, null, 4096);
        }

        public GZipResult Decompress(string lpSrcFolder, string lpDestFolder, string zipFileName,
        ...
        private bool WriteFile(FileStream fs, int fileLength, string lpFile, int bufferSize)
        {
            bool success = false;
        ...
        private string GetFolder(string lpFilePath)
        {
        ...
        private FileStream UnzipToTempFile(string lpZipFile, string lpTempFile, GZipResult result)
        {
            FileStream fsIn = null;
            GZipStream gzip = null;
            FileStream fsOut = null;
        ...
        private int GetCompressionPercent(long tempLen, long zipLen)
        {
